    ...
    if(!password || !repeatPassword){
      ...
      if(password !== repeatPassword){
        ...
        bcrypt.hash(password, null, null, function(err, hash) {
            if(err){
                res.status(500).send({message: "Error al encriptar las contraseña."});
            }else{
                user.password = hash;
            ...
            if(!userData){
                ...
                if(req.files){
                    ...
                    if(fileExt !== "png" && fileExt !== "jpg"){
                        res.status(400).send({message: "La extension de la imagen no es valida. (Exteniones permitidas: .png y .jpg)"});
                    }else{
                        ...
                        User.findByIdAndUpdate({ _id: params.id}, user, (err, userResult)=> {
                            if(err){
                                ...
                                if(!userResult){
                                    ...
                                    res.status(200).send({userName: fileName});
                                }
                            }
                        })
                    }
...
function signUpAdmin(req, res){
    ...
    if(!password){
      ...
      bcrypt.hash(password, null, null, (err, hash) => {
        if(err){
            res.status(500).send({ message: "Error la encriptar la contraseña."})
        }else{
            user.password = hash;
